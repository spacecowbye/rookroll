...
/* get_ms() returns the milliseconds elapsed since the function
   is first called. */
...
int get_ms()
{
	...
	ftime(&timebuffer);
	if (first_time == 0)
		first_time = timebuffer.time;
	r = (int)(timebuffer.time - first_time);
	r = (1000 * r) + timebuffer.millitm;
	return r;
}
...
int main()
{
	...
	printf("version 1.82, 1/19/23\n");
	printf("Copyright 2023 Tom Kerrigan\n");
	printf("\n");
